...
You need the usual Rust toolchain and Node.js.
...
The WebAssembly build uses Rust's `wasm32-unknown-unknown` target. This is the
standard Rust target for producing a standalone `.wasm` file that can run in the
browser.
...
If that command prints nothing, install the target once:
